...
class StorPerfMaster(object):
    ...
    def is_stack_created(self):
        return (self.stack_id is not None
            and self.heat_stack.get_status() == u'CREATE_COMPLETE')
    ...
    def create_stack(self):
        with open('storperf/resources/hot/storperf-agent.yaml', 'r') \
            as template_file:
            template=template_file.read()
        self.stack_settings.files = {"storperf-agent.yaml": template}
    ...
    def execute_workloads(self, metadata={}):
        ...
        if not self._test_executor.terminated():
            raise "ERROR: Job {} is already running".format(
