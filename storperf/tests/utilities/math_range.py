...
from storperf.utilities.math_range import MathRange as math
...
class MathRangeTest(unittest.TestCase):
    ...
    def test_empty_series(self):
        expected = 0
        data_series = []
        actual = math.range(data_series)
        self.assertEqual(expected, actual)

    def test_integer_series(self):
        expected = 11946
        data_series = [5, 351, 847, 2, 1985, 18, 96, 389, 687, 1, 11947, 758, 155]
        actual = math.range(data_series)
        self.assertEqual(expected, actual)
    ...
    def test_float_series_3_decimals(self):
        expected = 992.181
        data_series = [4.562, 12.582, 689.452, 135.162, 996.743, 65.549, 36.785]
        actual = math.range(data_series)
        self.assertEqual(expected, actual)

    def test_float_series_4_decimals(self):
        expected = 122985.3241
        data_series = [39.4785, 896.7845, 11956.3654, 44.2398, 6589.7134, 0.3671, 122985.6912]
        actual = math.range(data_series)
        self.assertEqual(expected, actual)

    def test_float_series_5_decimals(self):
        expected = 8956208.84494
        data_series = [12.78496, 55.91275, 668.94378, 550396.5671, 512374.9999, 8956221.6299]
        actual = math.range(data_series)
        self.assertEqual(expected, actual)

    def test_float_series_10_decimals(self):
        expected = 5984.507397972699
        data_series = [1.1253914785, 5985.6327894512, 256.1875693287, 995.8497623415]
        actual = math.range(data_series)
        self.assertEqual(expected, actual)

    def test_float_mix(self):
        expected = 60781.6245372199
        data_series = [60785.9962, 899.4, 78.66, 69.58, 4.93795, 587.195486, 96.7694536, 5.13755964, 33.333333334, 60786.5624872199]
        actual = math.range(data_series)
        self.assertEqual(expected, actual)
